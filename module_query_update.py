...
def init(start_date=None, end_date=None):    
    ...
    logger.info(f"五级分类数据生成初始化完成！查询时间段为：{start_date} 至 {end_date}")
    return config
...
def get_deloitte_rating_1(config):
    ...
    # 按 trade_date 分组并计算每日的百分位数阈值iles = [0.95, 0.85, 0.30, 0.07]      ###   设置百分位数阈值!!!    ###
...
#######################################################################
def daily_query_history(s,e):          #### 获取历史数据
    query_config=init(start_date=s, end_date=e)
    wanlian_df=get_saas_data(query_config)
    return wanlian_df

def daily_query_initialize(range_date):
    query_config=init(start_date=None, end_date=None)
    end_date = pd.read_sql("SELECT MAX(td) FROM model_st ms ;",query_config['istock_conn'])
    end_date=end_date.loc[0][0].strftime('%Y-%m-%d')
    new_date = range_date
